...
```bash
python3 "<claude-checkup-plugin-root>/skills/audit-authoring/scripts/validate_skill.py" <skill-dir>
```

Per the script-failure contract in `<claude-checkup-plugin-root>/reference/conventions.md`, surface stderr and
...
Severity (per the scale in `<claude-checkup-plugin-root>/reference/conventions.md`): Contradictions HIGH,
...
Follow the consent boundary in `<claude-checkup-plugin-root>/reference/conventions.md`: offer `chmod +x` for
