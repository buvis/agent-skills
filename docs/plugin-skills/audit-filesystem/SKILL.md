...
```bash
python3 "<claude-checkup-plugin-root>/skills/audit-filesystem/scripts/audit_filesystem.py"
```
...
`<claude-checkup-plugin-root>/reference/conventions.md`). Per the script-failure contract there: if it exits
...
Follow the consent/safety boundary in `<claude-checkup-plugin-root>/reference/conventions.md`. Group the
