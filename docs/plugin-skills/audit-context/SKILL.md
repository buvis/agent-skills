...
```bash
python3 "<claude-checkup-plugin-root>/skills/audit-context/scripts/audit_context.py" --mcp-tools <count>
```
...
`<claude-checkup-plugin-root>/reference/conventions.md`, surface stderr and stop if it exits non-zero.
