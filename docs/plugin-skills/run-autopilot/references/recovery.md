...
1. **Spend an approved rescue.** Otherwise run `python3 <autopilot-plugin-root>/skills/run-autopilot/scripts/fablectl.py <ledger> show <state.prd>`. When it reports `status == "approved"`, spend the rescue instead of stalling, **in this order** (each numbered write is durable before the next runs):
   a. **Queue the retry first** — `task-set-meta <task-id> <meta-json-file>` with payload `{"model": "fable", "escalation_reason": "fable_rescue", "escalated_from": "opus"}` (writes `state.tasks[i].model` directly, no separate mirror write), `task-set-status <task-id> pending`, append the id to `state.rework_task_ids`. These are Phase 6 step 4's existing requeue mechanics (`references/phase-review.md`) with `"fable"` as the tier — the gate reuses them, it does not add a dispatch path.
