...
- **changelog-check**: CI now fails when a skill directory under `skills/` is
  ...
  catching a shipped skill that never got a changelog entry before merge. The
  name must appear in exact case, and the check reads this repo's `skills/` and
  `CHANGELOG.md` from its own location, so it reports the same result from any
  working directory.
...
- **brief-portfolio**: `collect_ci` no longer reports a failed `actions/runs` fetch (a 500 or a rate-limit 403) as "no CI runs" with zero warnings; only HTTP 403/404 still read as Actions disabled (`ci: []`, no error), and every other failure lands a `ci:` entry in the repo's `errors` and leaves the `ci` key absent, which the page already renders as "not collected this run".
- **brief-portfolio**: a `skills.jsonl` row whose `ts` is not a string, does not parse as ISO 8601, has no timezone, or lies in the future no longer aborts the collect run after all the GitHub work; both metrics readers skip such rows and the run writes `data.json` from the valid ones, an unexpected skill-adherence reader failure costs only that tile (one `WARN skill_adherence:` on stderr), and the audit-cadence newest-day no longer lets `not-a-date` or a future date win, so that audit's nag fires again.
- **brief-portfolio**: `collect.py` no longer crashes the whole collect run when a repo's `gh api repos/<owner>/<name>` metadata call answers with a non-JSON or empty body; the failure now lands as a `meta:` entry in that repo's `errors` and collection continues, matching the existing HTTP-403 handling.
- **use-qwen**: `run_eval_harness.py` now refuses `inf`, `nan` and a non-numeric value for `--bound` and `--gate-bound` with one argparse error naming the flag, the value and the reason, so every deadline a run promises is finite; the test-only `gates.run_gates` driver is gone, the production attempt loop being the one gate orchestration.
- **use-qwen**: a description task whose `task_text` carries its acceptance bullets (or a pin or invariant opening a line with a patch marker) now vets, the leak refusal covering only the tdd `architecture` and anchors; a relative `repo`, an absolute, backslash or `..`-traversing read anchor, an anchor naming a writable or oracle path, and a declared `kind` that contradicts the classified writable count are refused by name; `spec.json` and `dispatch-references.json` are read as UTF-8 on every host; and a task directory without an integer prefix is refused with a `SpecError` naming it instead of a raw `ValueError`.
- **use-qwen**: an eval-harness attempt's usage now sums every assistant message of the transcript once (lines sharing a Claude Code `message.id` count as one) instead of reading the last message alone, a malformed transcript text block reads as an incomplete run instead of crashing the attempt, and a successful Claude edit whose result carries `is_error: false` now counts for first-edit timing.
- **use-qwen**: a tdd oracle that lives in a directory the template lacks now dispatches instead of ending in PREP_MISMATCH, an oracle overlay that changes nothing no longer aborts the run (the seal and the gate land an empty commit), a refused re-vet leaves no stale `ready` marker behind, the warmup list shares one gate deadline instead of a fresh one per segment, and every vet check holds the gate lock under its own label.
...
- **debrief-meeting**: escape every `<` in the collected JSON payload instead
  of only `</`, so a transcript turn containing `<!--<script>` can no longer
  move the HTML tokenizer into script-data-double-escaped state and blank the
  built page.
