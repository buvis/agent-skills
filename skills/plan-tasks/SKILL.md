...
```bash
bash <autopilot-plugin-root>/skills/plan-tasks/scripts/list-prds.sh
```
...
```bash
python3 <autopilot-plugin-root>/skills/run-autopilot/scripts/statectl.py dev/local/autopilot/state.json task-add <task-json-file>
```
...
`qwen_eligible` is persisted on **every** task `plan-tasks` creates. `/work` reads the field directly and does no re-judging — it routes per `qwen_eligible` + its own qwen infra preflight (see `<autopilot-plugin-root>/skills/work/SKILL.md`).
...
```bash
python3 <autopilot-plugin-root>/skills/run-autopilot/cli/__main__.py check-plan
```
