...
Scan the Codex setup, estimate token overhead per component, classify load behavior, and report with optimization recommendations.
...
```
Glob("~/.codex/projects/*/memory/MEMORY.md")
Glob("~/.codex/projects/*/memory/*.md")
```
...
Also parse `~/.codex/settings.json` field `mcpServers` and each project settings.json to list configured servers. Cross-reference: if a configured server has no `mcp__*` tools in the deferred list, it may not be connected.
...
```
Glob("~/.codex/plugins/cache/*/*/*/agents/*.md")
Glob("~/.codex/plugins/cache/*/*/*/skills/*/SKILL.md")
Glob("~/.codex/plugins/cache/*/*/*/commands/*.md")
```
...
|-------------|---------|-----------|
| Prose (AGENTS.md, agent descriptions, skill descriptions) | word_count x 1.3 | Prose tokenizes ~1.3 tokens per word |
...
|---------------|---------|----------|
| **Always loaded** | In context for every message | AGENTS.md files, MCP tool *names* (deferred tool list), plugin agent descriptions (in Agent tool definition), plugin skill/command names (in skill listing), memory files (MEMORY.md per project) |
...
─────────────────────────────────────────────────────────────────────────────
~/.codex/AGENTS.md                Config         Always loaded      X,XXX
...
- "Project AGENTS.md for inactive project: ~2K tokens - consider archiving"
