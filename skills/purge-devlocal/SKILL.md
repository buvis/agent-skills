...
compatibility: "Portable; a stdlib-only GC needing python3, plus rg for the reference check and the sibling review-prd-backlog skill in this repo for the post-apply link check. Optional: engram for the pre-trash harvest. One extra citer, the per-project memory store, is read only on hosts that keep one."
---
...
- Cross-skill script: `~/.agents/skills/review-prd-backlog/scripts/check_links.py`
  (post-apply dangling-reference verification, PRD 00081). Absent: say the
  inverse check did not run; never hand-roll a substitute.
- Optional: `engram` on PATH harvests a review satellite's findings before it
  is trashed. Absent: the script prints one WARN and trashes as usual; the
  `Verdict:` lines still reach the ledger, which is stdlib-only.
...
1. Dry-run and show the user the per-store summary:
   ...
   ```
   python3 ~/.agents/skills/purge-devlocal/scripts/purge_devlocal.py --all
   ```
...
2. Sanity-check the output. `prds/**`, keepers (capsule, decisions,
   ...
   **Unattended** (`CLAUDE_UNATTENDED=1`, or whatever marker the host sets for
   a batch run; when it sets none, treat a session that cannot prompt as
   unattended): never auto-trash a curated spike —
...
3. Pre-trash reference check (PRD 00081): before applying, verify nothing
   ...
   plus the per-project memory store, on hosts that keep one and only when
   purging that host's own config dir (Claude Code:
   `~/.claude/projects/<hash>/memory/` when purging `~/.claude`; no store, one
   citer fewer, and say so). Needles: the store-relative path for nested
   ...
   the memory store only. An empty result is unverified until a control
   pattern proves the command shape matches.
   ...
   `~/.agents/skills/review-prd-backlog/scripts/check_links.py --json`
...
4. Apply:
   ...
   ```
   python3 ~/.agents/skills/purge-devlocal/scripts/purge_devlocal.py --all --apply
   ```
...
Wire the apply run into whatever drains your backlog: the `autoclaude` batch
wrapper calls `--repo "$PWD" --apply` after a successful drain, so a store GCs
itself at batch end. Manual runs cover repos no such loop ever visits.

## Tests

```
python3 -m pytest ~/.agents/skills/purge-devlocal/scripts/test_purge_devlocal.py -q
```
