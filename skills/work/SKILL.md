...
- Files read from other skill dirs:
  - `~/.agents/skills/run-autopilot/scripts/statectl.py` - the sole `state.json`
    mutator; `/work` invokes it to append attempt entries and sync task status
    (never the Edit/Write tools)
  - `~/.agents/skills/run-autopilot/scripts/_walk_up.py` - run at every task
    start and at the handoff check
  - `~/.agents/skills/run-autopilot/prompts/de-sloppify.md` - its
...
1. **Claim the task** with the single compound verb (see Dashboard State Sync) — `task-start` sets `status: in_progress` in the state file, so this call IS the claim:
   ```bash
   python3 ~/.agents/skills/run-autopilot/scripts/statectl.py <state.json> task-start <task-id>
   ```
2. **Reset the per-task context-cap marker** so the autopilot PostToolUse hook fires once for THIS task, not once per Work phase. The hook also self-clears when the in-progress task id in `state.json` differs from the id stored in the marker file, but the explicit clear here is a belt-and-braces backstop in case state.json's task-id snapshot lags the actual task switch. Run the shared walk-up helper in `--clear-cap` mode — it resolves symlinks, walks up to the autopilot dir, and removes `<autopilot_dir>/.cap-fired` internally:
   ```bash
   python3 ~/.agents/skills/run-autopilot/scripts/_walk_up.py --clear-cap
   ```
...
```bash
python3 ~/.agents/skills/work/scripts/render_prompt.py ~/.agents/skills/work/references/tess-prompt.md \
...
```bash
python3 ~/.agents/skills/work/scripts/render_prompt.py ~/.agents/skills/work/references/tess-retry-prompt.md \
...
Render: one Bash call. **Task-authored prose never crosses the shell** — write it to a scratch file with the Write tool and pass `--set-file`; see § Passing values to render_prompt.py. Every interpolated path is `shlex.quote()`-d (or bash's `printf '%q '`) before it lands inside a `--set-cmd` value:
```bash
python3 ~/.agents/skills/work/scripts/render_prompt.py ~/.claude/agents/ivan.md \
...
| 4 | Backend, `qwen_eligible == true`, row 3 did not fire, and `python3 ~/.agents/skills/work/scripts/check_memory_pressure.py --max-level <ladder § Memory gate value>` exits non-zero | Claude at the task's ORIGINAL tier (`haiku` → Haiku, `sonnet` → Sonnet) — **skip the preflight probe**; stamp the attempt `preflight_outcome: null` and `qwen_excluded_reason: "memory_pressure"` (exit 1) or `"memory_probe_failed"` (exit 2) | `scripts/check_memory_pressure.py` |
...
- **Counter/latch update** happens later, at the step-5.5 gate — see that section. One rule lives here too, because a passing run never opens that reference: a qwen gate pass resets `qwen_gate_failures_consecutive` to 0, so the breaker trips only on two CONSECUTIVE qwen gate failures.
...
- `{{slop_catalog}}` from the `## What to remove` section of `~/.agents/skills/run-autopilot/prompts/de-sloppify.md` — read the file at dispatch time and inline the section verbatim. This keeps the deslop prompt as the single source of truth for slop patterns; when it grows entries, the next step-5.6 dispatch picks them up without a code change here.
...
2. Render the review prompt with one Bash call, every interpolated path `shlex.quote()`-d (or bash's `printf '%q '`) before it lands inside a `--set-cmd` value:
   ```bash
   python3 ~/.agents/skills/work/scripts/render_prompt.py ~/.claude/agents/pat.md \
     ...
     --set-file SIMPLIFICATION_MANDATE=~/.agents/skills/work/references/simplification-mandate.md
   ```
...
3. Dispatch via the sonnet runner (helper-script dispatch — the **Subagent Watchdog** applies):
   ```bash
   bash ~/.agents/skills/use-sonnet/scripts/sonnet-run.sh -f dev/local/tmp/review-task-<id>-prompt.md -o dev/local/tmp/review-task-<id>.md
   ```
...
2. **Land the whole transition in ONE `statectl` call.** Write the record from point 1 to `dev/local/tmp/attempt-task-<id>.json` with the **Write tool** (never a shell redirect — an attempt record carries quotes and newlines), then:
   ...
   ```bash
   python3 ~/.agents/skills/run-autopilot/scripts/statectl.py <state.json> task-done <task-id> dev/local/tmp/attempt-task-<id>.json
   ```
...
2. Resolve the autopilot dir and check for the marker:
   ```bash
   python3 ~/.agents/skills/run-autopilot/scripts/_walk_up.py --bash
   ```
