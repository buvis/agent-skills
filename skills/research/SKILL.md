---
name: research
description: Use when researching a topic via multi-source synthesis such as a literature review, competitive analysis, or technology comparison. Triggers on "research this", "investigate", "survey the landscape".
---
...
1. Clarify intent if the request is vague (1-2 questions max). In unattended
   mode, skip questions, research the request as stated, and note assumptions.
2. Launch a general-purpose research sub-agent with the prompt template below.
   Use the host's economical research-capable model by default and reserve its
   strongest reasoning model for genuinely hard synthesis.
