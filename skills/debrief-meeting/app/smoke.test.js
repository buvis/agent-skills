...
const PAYLOAD = {
  transcript: {
    ...
    speakers: [
      { id: 's0', name: 'Sam Okafor', words: 20, turns: 2, seconds: 40, share_words: 66.7,
        share_time: 66.7, wpm: 30, questions: 1, interruptions: 0, longest_turn: 25,
        first_t: 2, last_t: 120 },
      { id: 's1', name: 'Alex Rivera', words: 10, turns: 1, seconds: 20, share_words: 33.3,
    ...
    corrections: [{ kind: 'speaker-merge', from: 'Okafor, Sam', to: 'Sam Okafor' }],
    warnings: [],
  },
  extract: {
    ...
    decisions: [{ id: 'd1', title: 'Use Postgres', t: 52.5, status: 'made',
                  decision: 'Migrate to Postgres.', decider: 'Sam Okafor', context: 'Licensing.',
                  alternatives: [{ option: 'SQL Server', why_not: 'cost' }],
                  consequences: ['ops tuning'], confidence: 'high' }],
    actions: [{ id: 'a1', action: 'Talk to reporting', assignee: 'Alex Rivera',
                due: '2026-08-13', t: 12 }],
    questions: [{ q: 'Can reporting move?', asked_by: 'Sam Okafor', t: 12, answered: false }],
    risks: [{ risk: 'Reporting reads the old store', raised_by: 'Alex Rivera', t: 12,
    ...
    quotes: [{ text: 'Then we go with Postgres.', who: 'Sam Okafor', t: 52.5 }],
    quality: { on_topic_ratio: 0.95, notes: ['closed every item'] },
    dynamics: ['Sam chaired.'],
...
test('an action with no id stays ticked by its own text, not its position in the list', async () => {
  ...
  payload.extract.actions = [{ action: text, assignee: 'Sam Okafor', due: '2026-08-20', t: 30 }]
  ...
  },
  {
    ...
    mutate: (payload) => {
      payload.extract.followups = [
        { what: 'Duplicate followup text.', when: 'next sync', who: 'Sam Okafor' },
        { what: 'Duplicate followup text.', when: 'next sync', who: 'Sam Okafor' },
      ]
    },
    tab: 'Brief',
  ...
  },
  {
    ...
    mutate: (payload) => {
      payload.extract.risks = [
        { risk: 'Duplicate risk text.', raised_by: 'Alex Rivera', t: 12, severity: 'high',
          addressed: false },
        { risk: 'Duplicate risk text.', raised_by: 'Sam Okafor', t: 40, severity: 'medium',
          addressed: true },
      ]
    },
    tab: 'Insights',
  ...
  },
  {
    ...
    mutate: (payload) => {
      payload.extract.quotes = [
        { text: 'Duplicate quoted text.', who: 'Sam Okafor', t: 10 },
        { text: 'Duplicate quoted text.', who: 'Alex Rivera', t: 20 },
      ]
    },
    tab: 'Insights',
...
test('a zero-hit correction shows the not-found badge, distinct from an applied one', async () => {
  ...
  payload.transcript.corrections = [
    ...
    { from: 'Okafor, Sam', to: 'Sam Okafor', applied: 2 },
  ]
  const { doc, openTab } = render(payload)
