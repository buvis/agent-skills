...
compatibility: "Portable; a self-contained transcript parser and Svelte app needing only python3 and, to rebuild the template, npm. Parsing is stdlib-only, so a host that can run a script and write a file can run it."
---
...
- Path: `~/.agents/skills/debrief-meeting/assets/template.html` - the pre-built SPA. Missing
...
```bash
python3 ~/.agents/skills/debrief-meeting/scripts/parse.py /path/to/transcript.vtt
```
...
It canonicalises speaker names (`Rivera, Alex` / `A. Rivera` / `Alex Rivera
...
```bash
python3 ~/.agents/skills/debrief-meeting/scripts/build.py /path/to/transcript-debrief
...
npm --prefix ~/.agents/skills/debrief-meeting/app run build
cp ~/.agents/skills/debrief-meeting/app/dist/index.html ~/.agents/skills/debrief-meeting/assets/template.html
npm --prefix ~/.agents/skills/debrief-meeting/app test
```
