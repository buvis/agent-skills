...
def display_form(name: str) -> str:
    """Drop role suffixes and un-swap 'Rivera, Alex' into 'Alex Rivera'."""
    n = re.sub(r"\s*\([^)]*\)", " ", name)
...
def name_key(name: str) -> tuple[str, ...]:
    """Order-insensitive identity key: 'Rivera, Alex' == 'Alex Rivera'."""
    return tuple(sorted(w for w in re.findall(r"\w+", display_form(name).casefold())))


def _prefix_match(a: tuple[str, ...], b: tuple[str, ...]) -> bool:
    """'A. Rivera' vs 'Alex Rivera': same token count, every token a prefix pair."""
