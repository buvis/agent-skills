...
- Files read from other skill dirs:
  - `~/.agents/skills/review-blindly/references/rubric.md` - inlined into
...
1. `~/.agents/skills/use-codex/scripts/codex-run.sh` - executable
...
```bash
bash ~/.agents/skills/review-work-completion/scripts/gather-context.sh dev/local/tmp/review-tasks-{id}.md dev/local/tmp/review-prd-{id}.md
```

Incremental review (rework cycle) — prepend `--since <prior-cycle-head-sha>`:

```bash
bash ~/.agents/skills/review-work-completion/scripts/gather-context.sh --since <prior-cycle-head-sha> dev/local/tmp/review-tasks-{id}.md dev/local/tmp/review-prd-{id}.md
```
...
```bash
python3 ~/.agents/skills/review-work-completion/scripts/compute_mech_facts.py <changed file>...
```
...
```
Workflow({
  scriptPath: "/Users/you/.claude/workflows/review-fanout.workflow.js",
  args: { ... }
})
```
...
```bash
python3 ~/.agents/skills/review-work-completion/scripts/consolidate_findings.py \
...
```bash
python3 ~/.agents/skills/review-work-completion/scripts/check_review_file.py --review-file $PWD/dev/local/reviews/<review-file> --require-codex-guard
```
