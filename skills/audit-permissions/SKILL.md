...
- `~/.codex/settings.json` (source: **global**)
- `.codex/settings.json` in the current working directory (source: **project**)
...
- `Read(~/.codex/skills/**)` - read skill files
- `WebSearch` - no pattern, single capability
- `Write(./**/dev/local/**)` - scoped write path

## Step 3: Cross-reference with session usage

Check tool usage observations at `~/.codex/instincts/projects/`. The project registry at `~/.codex/instincts/projects.json` maps project hashes to names.
...
Also check the warden audit log at `~/.codex/warden-audit.jsonl` for Bash command usage. Each line:
```json
...
Read `~/.codex/warden.yaml` (user-level) and `.codex/warden.yaml` (project-level) if they exist.
