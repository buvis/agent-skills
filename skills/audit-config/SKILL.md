...
```bash
python3 "<claude-checkup-plugin-root>/skills/audit-config/scripts/audit_config.py"
```
...
in `<claude-checkup-plugin-root>/reference/conventions.md`: if it exits non-zero, surface stderr and stop --
...
Follow the consent/safety boundary in `<claude-checkup-plugin-root>/reference/conventions.md`: for CRITICAL
