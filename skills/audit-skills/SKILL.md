...
```bash
python3 ~/.codex/skills/create-skill/scripts/validate_skill.py <skill-dir>
```
...
Search the SKILL.md body for `@`-path patterns (e.g., `@~/.codex/skills/...` or `@./references/...`). These force-load files into context. **Warn** if found; recommend referencing skills by name instead.
...
Pattern adapted from `affaan-m/everything-Codex` `tests/ci/agent-instruction-safety.test.js`, which asserts hardcoded `(file, heading, patterns)` tuples per agent-facing file. This generalizes the principle to every skill via heuristic detection.
...
Use judgment: skills in the same domain (e.g., `audit-hooks` and `audit-plugins`) naturally share the word "audit" - that is fine. Flag only when the descriptions are so similar that Codex could not distinguish which skill to select.
