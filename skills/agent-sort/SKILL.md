---
name: agent-sort
description: Use when a project needs a trimmed Codex install instead of the full global setup, sorting skills/rules/hooks into DAILY vs LIBRARY with repo evidence. Triggers on "agent sort", "trim skills for repo", "daily vs library", "project install plan".
...
Read whichever manifests exist (`package.json`, `pyproject.toml`, `Cargo.toml`,
`pubspec.yaml`, or `go.mod`) with the host's file-reading tool.

## Parallel Review Passes

Dispatch parallel subagents (Agent tool with subagent_type=Explore) to split the review into these passes:

1. Skills — classify entries under `~/.codex/skills/` and any plugin-provided skills
2. Commands — classify entries under `~/.codex/commands/`
3. Rules — classify entries under `~/.codex/rules/`
4. Hooks and scripts — classify hook surfaces in `settings.json`, MCP health checks, helper scripts in `~/.codex/hooks/`
5. Extras — classify MCP configs, templates, and reference docs in `~/.codex/dev/local/`
...
- DAILY skills → keep enabled in `~/.codex/skills/`, or symlink/copy into the project's `.codex/skills/`
- DAILY rules → reference only matching language/framework rule sets in AGENTS.md
...
If the project already has a `.codex/` directory with selective installs, update that plan instead of creating another system.
