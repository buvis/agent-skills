...
- Personal skills (files read at runtime): `create-prd` (`SKILL.md` plus its `assets/` templates, the law for lens A)
- Plugin skill (`autopilot` plugin): `autopilot:plan-tasks` owns the budget and tier
  rules this gate checks against (its steps 4-4.7). Read them from the installed
  plugin. Without the plugin, skip the budget and tier questions and say so in the
  report; every other lens still runs.
...
- Frontmatter: only recognized fields (`catchup`, `rework_cap`, `design`, `design_gate`, `doubt_reviewer`, `consensus_engine`, `default_model`), valid values (`consensus_engine`: legacy | shadow | workflow, default legacy, parsed by run-autopilot Phase 0). Phase 0 defaults SILENTLY on anything invalid - a typo like `rework_cap: five` or `catchup: no` is a silent no-op (Non-blocking, but always flag).
...
- **Unattended feasibility**: sweep the WHOLE PRD, not just acceptance criteria, for human-in-the-loop indicators - "ask the user", "user decides/picks/confirms", "manually test", "validate with the user", a decision or Open Question deferred to implementation time. Any hit → Blocking (unattended hang), and it marks an incorrectly created PRD: create-prd's Unattended rule requires user input to be resolved in the attended phases before PRD creation. This gate is the last attended stop - resolve the point with the user in step 8 and edit the answer in, or HOLD.
