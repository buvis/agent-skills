...
1. **Global**: `~/.codex/settings.json` and `~/.codex/settings.local.json`
2. **Project**: `~/.codex/projects/<project-dir>/settings.json` and `settings.local.json`
3. **Local**: `.codex/settings.local.json` inside repo directories
...
```
Glob: **/settings.json in ~/.codex/projects/
Glob: **/settings.local.json in ~/.codex/projects/
```
...
Scan known repo roots for `.codex/settings.local.json`. Use the project directory names from `~/.codex/projects/` to derive repo paths (decode `-` to `/`, leading `-` to `/`). For each resolved path, check if `.codex/settings.local.json` exists there.

```
Glob: **/.codex/settings.local.json in ~/git/
```
