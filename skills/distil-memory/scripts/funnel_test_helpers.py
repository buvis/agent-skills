"""Scaffolding the funnel test modules share: the transcript-parser stubs every
one of them needs, plus the corpus fixtures, the fake `claude` CLI and the read
guards the distil modules are built on.

Deliberately not named `test_*`, so pytest imports it instead of collecting it."""
...
def _deliver(cmd, answer):
    ...
    is a failing `subprocess.run` stand-in (the suite defines several)."""
    if callable(answer):
        return answer(cmd)
