...
```
~/.codex/tasks/<session-id>/     # Task JSON files (1.json, 2.json...)
~/.codex/projects/<encoded-path>/<session-id>.jsonl  # Session data
```

Path encoding: `/Users/you/foo` → `-Users-you-foo`
...
```bash
bash ~/.agents/skills/restore-tasks/scripts/list-task-sessions.sh [project-path]
```
...
```bash
bash ~/.agents/skills/restore-tasks/scripts/dump-tasks.sh <session-id>
```
...
| Situation | Action |
|-----------|--------|
| No project data | Report "No Codex data for this directory" |
