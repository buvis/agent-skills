...
# -P resolves the link farm: this skill is reached as ~/.agents/skills/use-qwen,
# so a logical ../../.. lands on ~/.agents rather than the source repository.
REPO_ROOT="$(cd -P "$SCRIPT_DIR/../../.." && pwd)"
SKILL_MD="${QWEN_SKILL_MD:-$SCRIPT_DIR/../SKILL.md}"
INTEGRATION_MD="${QWEN_INTEGRATION_MD:-$REPO_ROOT/docs/plugin-skills/work/references/qwen-integration.md}"
...
for required in "$SKILL_MD" "$INTEGRATION_MD" "$TEST_SH"; do
    if [ ! -f "$required" ]; then
        echo "promote-default: cannot read $required" >&2
        echo "promote-default: set QWEN_SKILL_MD / QWEN_INTEGRATION_MD / QWEN_TEST_SH to override" >&2
        exit 1
    fi
done
