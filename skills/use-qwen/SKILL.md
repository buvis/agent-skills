...
compatibility: "Portable; needs the pi agent, a llama.cpp-compatible endpoint, and bash. Shares the dispatch contract with the sibling use-codex skill in this repo, and its helper scripts locate their own siblings, so any host that can run a shell can drive it."
---
...
Background dispatch and waiting (TaskOutput-only waiting), following up, error handling, and the always-use-`-f` prompt rule are defined once in the sibling `use-codex` skill, at `~/.agents/skills/use-codex/references/dispatch-contract.md`. Read it before dispatching; it applies verbatim to this skill. On Claude Code the `autopilot` plugin owns `use-codex` and carries the same file. Without either, follow the background-dispatch rules stated inline below (local inference is slow, so the background-dispatch guidance matters even more here).
...
```
~/.agents/skills/use-qwen/scripts/qwen-run.sh --register-model -P <provider> [--name "..."]
```
...
2. Run WITHOUT `--commit` first:
   ```
   ~/.agents/skills/use-qwen/scripts/run-eval.sh -P <provider> -m <model-id> --tasks <manifest>
   ```
...
```
~/.agents/skills/use-qwen/scripts/promote-default.sh <model-id>
```
...
~/.agents/skills/use-qwen/scripts/qwen-run.sh -f /tmp/qwen-prompt.txt

# Read-only analysis
~/.agents/skills/use-qwen/scripts/qwen-run.sh -R -f /tmp/qwen-prompt.txt

# Structured JSON output captured to a file
~/.agents/skills/use-qwen/scripts/qwen-run.sh -j -o /tmp/result.jsonl -f /tmp/qwen-prompt.txt

# Override model
~/.agents/skills/use-qwen/scripts/qwen-run.sh -m other-model -f /tmp/qwen-prompt.txt
```

Run `~/.agents/skills/use-qwen/scripts/qwen-run.sh --help` for all options.
