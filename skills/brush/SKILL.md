...
compatibility: "Portable; the git hygiene core needs only python3, git and the skills it orchestrates. Two enrichments are Claude Code only and skip themselves elsewhere: the away-from-keyboard notify ping, and the memory-promotion half of phase 5."
...
- Optional: `gh` (PR-merge proof; absent = gone-branch deletes demote to
  ASK unless the squash probe proves the merge), a notifier script at
  `~/.claude/hooks/notify.py` (Claude Code installs only; absent = no ping).
...
1. Run `python3 ~/.agents/skills/brush/scripts/collect_facts.py --repo <cwd>`.
2. STOP and say why if: JSON has `refusals` (not a work-tree, or a dotfiles
   bare repo whose work-tree is `$HOME`),
...
4. Read `~/.agents/skills/brush/references/hygiene-rules.md` before phase 2.
...
5. **Instructions** (full only): run the `manage-agents-md` skill, then
   **promote load-bearing memories** when the host keeps a per-project memory
   store (Claude Code: `~/.claude/projects/<hash>/memory/`; no store, no
   promotion - say so in the report and move on): scan it for
   `feedback`/`project` entries whose
...
`python3 -m pytest ~/.agents/skills/brush/scripts/test_brush_scripts.py -q`
