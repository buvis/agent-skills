---
name: audit-security
description: Use when auditing ~/.codex/ for security anti-patterns. Flags loose permissions, hook injection, risky MCP, hardcoded secrets. Triggers on "audit security", "security audit", "check config security", "scan for vulnerabilities", "security check".
compatibility: "Codex-specific; reads ~/.codex configuration, sessions, or hooks and is not projected to Claude Code."
---

# Audit Security

Static security scan of `~/.codex/` configuration files.

## Step 1: Run scanner

```bash
python3 ~/.codex/skills/audit-security/scripts/scan.py
```
