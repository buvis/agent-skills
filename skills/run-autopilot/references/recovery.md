...
1. **Spend an approved rescue.** Otherwise run `python3 ~/.agents/skills/run-autopilot/scripts/fablectl.py <ledger> show <state.prd>`. When it reports `status == "approved"`, spend the rescue instead of stalling, **in this order** (each numbered write is durable before the next runs):
