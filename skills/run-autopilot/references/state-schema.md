...
```json
{
  ...
  "repo_root": "/absolute/path/to/the/repo/being/worked",
  "design_mode": "run",
  "design_gate": "user",
  ...
  "codex_probe": {"batch_id": "202603161000", "verdict": "healthy", "backend": "codex", "detail": null, "checked_at": "2026-03-16T10:45:00Z"},
  "qwen_preflight": {"batch_id": "202603161000", "verdict": "healthy", "detail": null, "checked_at": "2026-03-16T10:45:00Z"},
  "stall_reason": {
  ...
  "doubts_rubric_verdicts": [
    ...
    {"rule_id": "D2", "verdict": "pass"},
    {"rule_id": "D3", "verdict": "pass"},
    {"rule_id": "D4", "verdict": "fail"},
...
| `tasks` | object[] | Canonical task list for the current PRD: `{"id": "<task-id>", "name": "...", "status": "pending\|in_progress\|completed", "description"?: string, "blocked_by"?: int[], "model"?: "haiku\|sonnet\|opus\|fable", "attempts"?: object[]}`. Include `id` — state readers (tracon, the review phase, `rework_task_ids` targeting) key on it. The two optional entry fields `description` and `blocked_by` are detailed in the `tasks[].description` and `tasks[].blocked_by` rows below. |
| `tasks[].model` | enum? | Per-task model tier: `"haiku"`, `"sonnet"`, `"opus"`, `"fable"`. `"fable"` (PRD 00076) is reachable **only** via the Fable rescue gate (`references/recovery.md` § Rework escalation exhausted, on a human-approved ledger entry), never by the plan-tasks tier classifier and never as a session model. Set by plan-tasks step 4.7's tier classifier and rewritten by /run-autopilot Phase 6 escalation (see `references/phase-review.md` Phase 6). The dashboard, the review phase, and `/work` read this field. When absent (plans created before PRD 00025), `/work` omits `model` and subagents inherit the session model. |
| `tasks[].attempts` | object[]? | Per-task execution log: `{"attempt": int, "model": string, "outcome": "completed"\|"aborted"\|"review_flagged"\|"rework_failed"\|"escalated", "review_cycle": int\|null, "cause": string\|null, "implementor": "claude"\|"gemini"\|"qwen"\|"codex", "preflight_outcome": "healthy"\|"pi_missing"\|"endpoint_unreachable"\|"model_id_missing"\|"completion_failed"\|null, "pipeline"?: "minimal"\|"lean"\|"full", "self_deslop"?: string, "escalation_reason"?: "gate_failure"\|"review_flag"\|"fable_rescue", "escalated_from"?: "qwen"\|"haiku"\|"sonnet"\|"opus"\|"codex", "diagnosis"?: "spec_gap"\|"solid_spec", "repair_used"?: true, "breaker_skipped"?: true, "qwen_gate_failed"?: true, "qwen_excluded_reason"?: "memory_pressure"\|"memory_probe_failed", "codex_no_edit"?: true, "codex_no_edit_probe_exit"?: int}`. Records each Work pass on the task. Appended by `/work` at task exit (success or abort). The `implementor` field reflects what actually dispatched (NOT what step 3's routing table initially picked — a qwen pick that fell back to Claude on preflight failure records `"claude"`). The `preflight_outcome` field is set only for attempts on qwen-eligible tasks (`state.tasks[i].qwen_eligible == true` at attempt start); for non-qwen-eligible attempts (UI tasks, `opus`-tier tasks, backend tasks where `qwen_eligible` is `false` or absent) it is `null`. A qwen-eligible attempt that the breaker (row 3) or the memory-pressure gate (row 4) rerouted also records `null`, because the probe never ran. The `pipeline` field records the tier-gated pipeline depth the attempt ran — `"minimal"`/`"lean"`/`"full"` for `haiku`/`sonnet`/`opus`, and `"full"` for `fable` (the rescue rung runs the deepest pipeline, like `opus`; absent/legacy `model` is treated as `sonnet` → `"lean"`), written at every task exit (PRD 00044) — the `?` in the signature marks read-side tolerance for pre-00044 rows only; new writes always include `pipeline`. **Cardinality (PRD 00065):** today `/work` appends exactly one entry per task at exit. Under in-loop escalation the rule widens to one entry per rung/dispatch-group: each rung that dispatched writes its own entry when the task escalates away from it or exits on it. `attempt` numbers stay sequential across rungs (`len(existing)+1`). This is a widening, not a break — a task that never escalates still writes exactly one entry, byte-identical to today. **Six escalation fields (PRD 00065, all optional, absence = legacy / not-applicable):** `escalation_reason` (`"gate_failure"` stamped by `/work` on the entry of the rung it escalated INTO (in-loop diagnosis); `"review_flag"` stamped by phase-review Phase 6 on its escalation path; `"fable_rescue"` (PRD 00076) stamped by the Fable rescue gate on the entry of the human-approved fable retry, always paired with `escalated_from: "opus"` and an entry `model` of `"fable"` (the rescue rung — the one tier `/work` never selects on its own). Absent = none of the three (legacy / no escalation)); `escalated_from` (string?; normally a tier — on the entry of the rung escalated INTO, the immediate-prior rung it came from; the one exception is `"codex"` (PRD 00077), which names a BACKEND rather than a tier — stamped on the Claude entry a codex capability failure escalates into, the others are all tiers. Absent on non-escalated entries); `diagnosis` (string?; the 2nd-failure diagnosis verdict, stamped on the entry of the rung that was diagnosed); `repair_used` (bool?; `true` stamped on the entry of the same-tier attempt that ran after a task-description repair; absent otherwise, never written `false`); `breaker_skipped` (bool?; `true` stamped on a qwen-eligible attempt the breaker rerouted off qwen — to codex (PRD 00077) or to Claude at original tier (no preflight probe); absent otherwise); `qwen_gate_failed` (bool?; `true` stamped on an `implementor:"qwen"` entry whose step-5.5 gate FAILED — the durable signal the qwen capability breaker keys on; absent otherwise). `qwen_excluded_reason` (string?, PRD 00075; stamped on a qwen-eligible attempt the memory-pressure gate (`model-ladder.md` § Memory gate) rerouted off qwen — to codex (PRD 00077) or to Claude at original tier — `"memory_pressure"` when the gate probe exited 1 (under pressure), `"memory_probe_failed"` when it exited 2 (probe failed); absent otherwise, including on qwen dispatches, breaker-skipped attempts, and non-qwen-eligible tasks. **Attempt-level** — distinct from the plan-time `tasks[].qwen_excluded_reason` field described below, which records why plan-tasks excluded a task from qwen routing at plan time, not why a dispatch-time gate rerouted it). **Codex no-edit fields (PRD 00077, both optional):** `cause` gains the value `"codex_no_edit"` — the no-edit infra arm, stamped when the codex helper exited 0 with a non-empty output file but produced no working-tree change (the hook-blocked, prose-only case); classified as INFRA, so the attempt falls back to Claude at the task's own tier with no `escalation_reason` stamp. `codex_no_edit` (bool?; the LATCHED detector result, captured at the dispatch boundary — `work` SKILL.md step 4, before step 5's `git add` — by comparing the task's own file slice porcelain before and after the codex dispatch; step 5.5 reads this latched flag and never re-runs the detector. Absent on non-codex attempts and on attempts written before PRD 00077). `codex_no_edit_probe_exit` (int?; the detector's own exit code, recorded ONLY when the detector exited non-zero — a non-zero exit means the probe is INDETERMINATE, not "no change": `codex_no_edit` is NOT latched and the attempt continues on the capability path normally. Exists so a broken detector is visible in the record instead of silently reclassifying every real codex capability failure as infra. Absent when the detector ran cleanly and on attempts written before PRD 00077). The `"escalated"` outcome value is stamped on the entry for a rung the task was reset away from and escalated up; the final rung's entry carries the real terminal outcome (`completed`/`aborted`), and the escalated-away entry keeps its true `implementor` (a qwen rung escalated to Sonnet keeps `implementor:"qwen"`; likewise, a codex attempt records `model` at the task's own tier, e.g. `"sonnet"`, with `implementor: "codex"` carrying the backend identity — the same split the qwen rung already uses). See `work/references/attempt-logging.md` for field semantics, the atomic write procedure, and the attribution row-ownership table. Outcome upgraded later by /run-autopilot Phase 6: `review_flagged` at the start of escalation (step 2), then `rework_failed` if the chain exhausts at `opus` (or at `fable`, when a rescue attempt was spent — step 3's escalation-exhausted branch). Rework reads the last entry's `model` to determine the next tier. Absent on plans created before PRD 00025; `implementor`/`preflight_outcome` absent on attempts written before PRD 00031; `self_deslop` absent on attempts written before PRD 00041; `pipeline` absent on attempts written before PRD 00044; the six escalation fields and the `"escalated"` outcome value absent on attempts written before PRD 00065; the attempt-level `qwen_excluded_reason` absent on attempts written before PRD 00075; `codex_no_edit` and `codex_no_edit_probe_exit` absent on attempts written before PRD 00077. |
...
| `tasks[].estimated_tokens` | int? | Per-task input-token budget estimate. Set by plan-tasks step 4.7's tier classifier; persisted directly in `state.tasks[]`. Absent when plan-tasks did not record a budget estimate. |
| `tasks[].est_context_peak` | int? | Per-task estimated peak working-context size. Used to predict subagent dispatch budget headroom. Set by plan-tasks step 4.7's tier classifier; persisted directly in `state.tasks[]`. Absent when plan-tasks did not record a context estimate. |
| `tasks[].qwen_eligible` | bool? | Qwen routing eligibility (PRD 00032, widened by PRD 00019): backend (not UI) AND tier ∈ {`haiku`, `sonnet`} AND `files_touched <= 3` AND no public-contract edit. Set by plan-tasks step 4.7's tier classifier; persisted directly in `state.tasks[]`. `/work` reads this field for routing; absent (legacy plans) is treated as `false`. |
| `tasks[].qwen_excluded_reason` | string? | Why plan-tasks step 4.7 excluded the task from qwen routing: `"ui"`, `"tier"`, `"files"`, or `"contract"` (first failing condition in that order). Set by plan-tasks step 4.7's tier classifier on every ineligible task (PRD 00019); persisted directly in `state.tasks[]`. Absent on eligible tasks and legacy plans — the Phase 9 Implementor Mix render counts an ineligible task with no reason as `unknown`, never an error. |
| `tasks[].description` | string? | Free-form detail about what the task must achieve, beyond the one-line `name`. Validated by `cli/schema.py`: optional, and when present must be a `str` (any other type — including `bool` — is rejected as `tasks[<i>].description`). Absent on tasks that carry only a `name`. |
| `tasks[].blocked_by` | int[]? | Integer references to the tasks this one waits on. Validated by `cli/schema.py`: optional, and when present must be a `list` whose every element is a plain `int` — `bool` elements are rejected, matching the bool-is-not-int rule the int fields already use. An empty list is valid. Absent on tasks with no dependencies. |
...
| `repo_root` | string? | Absolute path to the git repo containing the work commits (`git rev-parse --show-toplevel`), captured at Phase 3 start alongside `work_start_sha`. Usually equals the project root; differs when the work repo is nested under a non-git project root (e.g. `<autopilot-plugin-root>/skills/run-autopilot` under `~/.claude`). Read by the review session to run `git` (diff gathering, `head_sha` capture) in the right repo. Cleared by Phase 9 step 10 on PRD-to-PRD reset. Optional: absent on older plans; consumers fall back to the project root. |
...
| `codex_probe` | object? | Batch-scoped codex health probe result (PRD 00077): `{"batch_id": "<state.batch.id>"\|"no-batch", "verdict": "healthy"\|"unhealthy", "backend": "codex"\|"copilot", "detail": "<one-line cause>"\|null, "checked_at": "<ISO 8601>"}`. Written once per batch by the codex batch health probe (`work` SKILL.md § Batch codex health probe) and read by the step-3 routing interception before any codex dispatch. Batch-scoped with the same LAZY reset `qwen_breaker` uses: `/work` re-probes and overwrites the slice when `codex_probe.batch_id != state.batch.id` (fallback id `"no-batch"` for a manual run with no `state.batch`); a match reuses the cached verdict and never re-probes. Writers MAY carry additional descriptive keys beyond these five (the live probe also stamps `model` and `note`); readers ignore unknown keys. Absent on legacy state files: readers treat absence as "not yet probed", never an error. |
| `qwen_preflight` | object? | Batch-scoped qwen preflight probe result (PRD 00093), mirroring `codex_probe`'s shape and idiom: `{"batch_id": "<state.batch.id>"\|"no-batch", "verdict": "healthy"\|"pi_missing"\|"endpoint_unreachable"\|"model_id_missing"\|"completion_failed", "detail": "<the failing check's own detail string>"\|null, "checked_at": "<ISO 8601>"}`. `verdict` reuses the four existing `preflight_outcome` strings directly — no separate `"unhealthy"` wrapper. Written once per batch by `work` SKILL.md § Qwen infra preflight (step 3 routing row 5) and read there before every qwen-eligible dispatch. Batch-scoped with the same LAZY reset `codex_probe`/`qwen_breaker` use: `/work` re-probes and overwrites the slice when `qwen_preflight.batch_id != state.batch.id` (fallback id `"no-batch"`); a match reuses the cached verdict and never re-probes. **Re-probe trigger is scoped to backend-health signals, not task-outcome signals** — an ordinary step-5.5 gate failure on a qwen attempt does NOT invalidate the cache (that is the one-shot-to-Sonnet escalation's job); only a Subagent Watchdog hang/lost-result on the qwen dispatch itself, or an infra-shaped exit from `qwen-run.sh` at real dispatch time despite a cached `"healthy"` verdict, deletes the field via `statectl del qwen_preflight`, so the next qwen-eligible task's batch-scope check re-probes. Absent on legacy state files: readers treat absence as "not yet probed", never an error. |
| `stall_reason` | object? | Signals a pending recovery action. Three shapes: `{"stalled": "oversized_task", "task": "<id>", ...}` (written by `/plan-tasks` when a task can't be split below budget — carries `estimated_tokens?: int`; AND by `autopilot_context_cap_hook.py`'s livelock guard on a second consecutive cap fire for the same in-flight task — carries `total_input_tokens: int`; both trigger a stall move to `dev/local/prds/hold/`); `{"stalled": "subagent_prompt_overrun", "task": "<id>", "prompt_bytes": int}` (written by `/work` Subagent Dispatch Budget when an assembled subagent prompt exceeds 50K after one trim pass — triggers Phase 0's **replan procedure**, not a stall move; see `replan_count` and `/run-autopilot` Phase 0 "Handle Work-phase abort"); `{"stalled": "escalation_exhausted", "task": "<id>"}` (written by `/run-autopilot` Phase 6 when a task at tier `opus` fails rework — Phase 6 performs its own stall move inline); `{"stalled": "taskcreate_failed", "detail": "<error>"}` (written by `/plan-tasks` step 4 when a `task-add` call fails mid-plan, after it rolls back the tasks it already created — PRD 00084; the `taskcreate_failed` value is a retained legacy key name, not a live tool reference; Phase 2 folds any non-`oversized_task` stall into its `plan_tasks_fail` PAUSE / loop re-invoke-once path, and `/plan-tasks` clears this marker itself on a successful retry). **Merged** (not replacing) into state by the writer, then **cleared** by `/run-autopilot`. Lifecycle handoff: Phase 2 handles `oversized_task` inline with a stall move; Phase 0 of the next session handles `subagent_prompt_overrun` via the replan procedure (no stall move — the PRD stays in `wip/`, planning re-runs with `replan-context.md`); Phase 6 handles `escalation_exhausted` inline (it owns the rework path and does its own move + clear before signaling — Phase 0 should never see this in normal operation, and treats it as crash recovery if it does). The context cap no longer writes `stall_reason` on a normal overrun — it ROTATES (see `cap_rotations`); only its livelock guard escalates to `oversized_task`. Absent in normal operation. |
...
| `contract_card` | string? | Compact re-anchor card (PRD 00087 R1): current step, active invariants, next gate. Written by `/run-autopilot`, `/work`, and `/review-work-completion` at each gate transition (via `statectl set`, in the same write that advances `phase`/`next_phase`). Read by the `reinject_contract_card.py` SessionStart(compact) hook, which re-injects it after a within-session compaction. Interactive runs use a scratch `dev/local/autopilot/contract-card.md` instead. Absent until the first gate transition writes it. Free-form; not parsed by any consumer. |
...
| `batch.completed_prds` | object[] | PRDs finished so far: `{"filename", "cycles", "autonomous_decisions", "escalated_decisions", "tasks_completed", "tasks_total"}`. Appended by `statectl.py`'s `complete-prd <prd-filename>` verb, which computes the entry from the state being closed (`cycles` = `cycle`; `autonomous_decisions` = count of `autonomous_decisions` entries the batch report's "Autonomous Decisions" table renders - dicts whose `type` is not `"assumed-ambiguity"` with at least one non-empty cell among cycle / issue-or-question / severity / action / reason-or-resolution, per `render_report.is_autonomous_row`; `escalated_decisions` = count of `deferred_decisions` entries whose `status` is neither `"pending"` nor `"deferred"`, mirroring `render_report._is_pending`'s exclusion rule exactly; `tasks_completed`/`tasks_total` copied as-is) and resets `batch.parks_consecutive` to 0 in the same write. |
...
| `batch.plugin_versions` | object? | Enforcement-plugin version pin (PRD 00086 R3): `{"aegis@buvis-plugins": "<v>", "warden@buvis-plugins": "<v>"}`, recorded at batch selection (Phase 0 step 3) from `~/.claude/plugins/installed_plugins.json`. The loop's plugin-pin preflight (`plugin_drift` in `cli/loop.py`, since PRD 00106) compares it against the currently-installed versions before each session launch and halts the batch loud on any drift (never runs on enforcement code that auto-updated mid-batch — sanctioned-halt row 11). Preserved across the per-PRD reset (lives under `batch`); a genuine batch-identity rollover re-pins from the current install. Absent on pre-R3 batches → the preflight no-ops (an unpinned batch is never blocked). |
...
`state-write-failed` (PRD 00051 task 9) is the halt signal for a broken state boundary — one that survives past `state.json` itself, so the recovery path must not depend on the thing that just failed: it is written without importing `cli/`, without a lock, and without schema validation. Two writers: a loop-mode session whose `statectl` write fails twice (retry, then `autopilot restore`, then this marker — `SKILL.md` § State Management) records the human-diagnosable failure at the point of the broken write; the cap hook's rotation/oversized-stall writers record it the same way when their own `cli.state.transaction` call cannot even run (import failure) or raises. The wrapper checks for it FIRST, ahead of every other branch, and does not need `state.json` to be readable to act on it. Unlike every other marker here, it is deliberately NOT consumed: the wrapper halts loud and leaves it in place, because the human clearing it is the only party who can confirm the state boundary itself is fixed.
...
A single JSON **object keyed by PRD filename** (the `00XXX-….md` basename, exactly as `state.prd` carries it):

```json
{
  "00076-route-builds-sonnet-first-with-fable-rescue-v1.md": {
    "status": "requested",
    "task_id": "task-uuid-8",
    "task_name": "Wire approval consumption",
    "requested_at": "2026-07-21T23:49:20Z",
    "batch_id": "202607202320",
    "justification": {
      "problem": "one-line what the task could not achieve",
      "attempts": "haiku(2) -> sonnet(2) -> opus(2), last outcome rework_failed",
      "impact": "one-line what stays broken if it is never done"
    },
    "decided_at": null,
    "consumed_at": null
  }
...
| Field | Type | Description |
|-------|------|-------------|
| `status` | enum | Exactly one of `"requested"`, `"approved"`, `"rejected"`, `"consumed"`. The only legal transitions are `requested → approved`, `requested → rejected`, and `approved → consumed`; `rejected` and `consumed` are **terminal**. No other transition is legal, and the table is enforced in `fablectl.py`, not in prose. |
| `task_id` | string | The blocked task's id at request time. **Forensic only** (see below). |
| `task_name` | string | The blocked task's `state.tasks[i].name` at request time. **Forensic only** (see below). |
| `requested_at` | string | ISO 8601 UTC (`%Y-%m-%dT%H:%M:%SZ`) stamped when the request was recorded. |
| `decided_at` | string? | Same format, stamped when the operator's `approved`/`rejected` decision landed. `null` until it happens. |
| `consumed_at` | string? | Same format, stamped when the approval was spent on a Fable dispatch. `null` until it happens. |
| `batch_id` | string | `state.batch.id` of the batch that hit the exhaustion. Forensic: an approval landing after that batch drains is still valid, because the ledger outlives the batch. |
| `justification` | object | Why the rescue is warranted, with three **required** keys: `problem` (one line on what the task could not achieve), `attempts` (the rung-by-rung history, e.g. `haiku(2) -> sonnet(2) -> opus(2), last outcome rework_failed`), and `impact` (one line on what stays broken if it is never done). A justification missing any of the three is rejected (exit 1). |

**Latch: one request per PRD, ever.** A key present for a PRD, whatever its status, blocks writing a new request for that PRD. Absence of the file, or absence of the key, means no request exists.

**`task_id` / `task_name` are forensic only.** Neither routes the retry: the stall handler clears `state.tasks` and the un-parked PRD replans from scratch, so no id or name recorded at request time survives. The retry is anchored to the PRD's next ladder exhaustion (`references/recovery.md` § Rework escalation exhausted).

**Reader tolerance.** An absent file means "no requests" and is never an error; `fablectl.py` creates it (and `ledger/`) on the first write. Unreadable JSON IS an error, not a silent reset: the file is never overwritten blind.

### `fablectl.py` - the sole writer

`skills/run-autopilot/scripts/fablectl.py` is the only writer of the ledger (same advisory-lock + temp-file + `os.replace` idiom as the sibling `statectl.py`). Every verb takes the exclusive lock for the whole read-modify-write, so the latch check and the write are one atomic step.

```
python3 fablectl.py <ledger.json> request <prd> <task_id> <task_name> <batch_id> <justification-json>
python3 fablectl.py <ledger.json> decide  <prd> approved|rejected
python3 fablectl.py <ledger.json> consume <prd>
python3 fablectl.py <ledger.json> show    <prd>
```

- `request` - writes the entry with `status: "requested"` and `requested_at` = now. **Exit 3 when a key for `<prd>` already exists, whatever its status** (the one-request-per-PRD-ever latch). A `<justification-json>` object missing `problem` / `attempts` / `impact` is exit 1.
- `decide` - **exit 3 unless the entry exists AND `status == "requested"`**; otherwise sets the new status and `decided_at`.
...
```
mv dev/local/autopilot/ledger/fable-requests.json.bak dev/local/autopilot/ledger/fable-requests.json
```
...
```
python3 <autopilot-plugin-root>/skills/run-autopilot/scripts/fablectl.py dev/local/autopilot/ledger/fable-requests.json decide <prd> approved
```
...
Governs which model launches a `build` (or empty, first-launch) phase session. Source: `build_model` in `cli/routing.py` (moved out of the bash wrapper by PRD 00106), called from the CLI loop's per-phase launch routing. This closes the promotion-signal half of the PRD's Phase 0 acceptance criterion "Define `fable_request` and promotion-signal fields in state-schema.md" (see § Fable rescue ledger above for the `fable_request` half).

**Precedence.** The `_AUTOPILOT_MODEL_BUILD` env override wins over everything: it is a kill switch — `route()` in `cli/routing.py` returns it without ever calling `build_model`, so no signal below can fire. Absent an override, any promotion signal firing launches `claude-opus-5[1m]`; with no override and no signal, the launch is `claude-sonnet-5[1m]`. Review-phase sessions always launch `claude-opus-5[1m]` at `xhigh` effort, and done-phase sessions always launch `claude-sonnet-5[1m]` at `medium` effort - separate branches of the same routing table, untouched by this per-PRD logic.
...
3. **Prior stall**, in either form:
   - 3a. Live: `state.stall_reason != null`.
...
| planning (build sub-step) | `state.tasks` contains pending or completed tasks (see `references/phase-build.md` Phase 2) |
| work (build sub-step) | All tasks completed, none pending (see `references/phase-build.md` Phase 3) |
