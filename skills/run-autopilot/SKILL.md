...
- Files read from other skill dirs:
  `~/.agents/skills/review-work-completion/scripts/await_reviewer_outputs.py`
  (Watcher keep-alive)
...
- **Autopilot (state.json world):** write the body to `dev/local/autopilot/contract-card.md` with the **Write tool**, then load it with one call:
  ```bash
  python3 ~/.agents/skills/run-autopilot/scripts/statectl.py <state.json> set-contract-card dev/local/autopilot/contract-card.md
  ```
