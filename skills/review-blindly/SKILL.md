...
- Personal skills: `use-sonnet` - the reviewer itself runs via
  `~/.agents/skills/use-sonnet/scripts/sonnet-run.sh` (and the `claude` CLI it
  wraps)
- Files read from other skill dirs:
  `~/.agents/skills/review-work-completion/scripts/check_review_file.py` - hard
...
2. **Gate the file (PRD 00016) and fail loud:**
   ```bash
   python3 ~/.agents/skills/review-work-completion/scripts/check_review_file.py --review-file dev/local/reviews/<prd>-blind-review.md
   ```
