...
Read `~/.codex/plugins/installed_plugins.json`. Parse it to build the authoritative list of installed plugins. Each key in `.plugins` is `{name}@{org}`. Extract:
...
```
Glob("~/.codex/plugins/cache/*/*/*/.codex-plugin/plugin.json")
```
...
For each version directory (the parent of `.codex-plugin/`), run:
...
```
Grep("~/.codex/projects/", pattern: "{plugin_name}")
```
...
────────────────────────────────────────────────────────────────────────────────
superpowers             Codex-plugins-official    2          5.0.7      12MB
warden                  buvis-plugins              1          0.6.2       4MB
pr-review-toolkit       Codex-plugins-official    1          unknown     2MB
