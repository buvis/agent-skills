...
compatibility: "Portable; ships no scripts and needs only git, rg, and a host that can run several read-only auditors. Richer with the git-ferry and ponytail plugins, and it names a fallback for each."
---
...
Look for a project config the invoking skill or repo provides (an `evolution.md` under the host's project config directory, an `assess-*` project skill, or an AGENTS.md section). It supplies:
...
6. **Simplification & librarization** — duplicated primitives (the same helper implemented twice, diverging), one-off reimplementations of things a shared helper/library should own, speculative abstractions with one implementation to delete, and clusters that multiple call sites (or multiple downstreams) reinvent and that should be extracted into a library/shared module. Pairs with lens 5: what downstreams reimplement is a librarization signal. Include declared debt: harvest `ponytail:` markers (ponytail:ponytail-debt skill if present, else `rg -n "ponytail:"`) - each names a deliberate ceiling and its upgrade trigger; flag markers whose trigger has since fired as refactor candidates.
