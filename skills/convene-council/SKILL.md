...
compatibility: "Portable decision workflow; requires a host that can isolate and run three sub-agents or equivalent model sessions."
---

# Convene Council

Convene four advisors for ambiguous decisions: the in-context assistant
...
Launch them through the host's sub-agent mechanism in one parallel batch. Each
gets only the prompt below (question, compact context, strict role; no
conversation history, per the Roles-table rule). If the host cannot isolate
sub-agents, run the same prompts in separate model/CLI sessions.
...
- save a memory only through the active host's documented memory mechanism,
  and only if the outcome is durable guidance for future sessions
