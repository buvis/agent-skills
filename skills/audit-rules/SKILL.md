...
## Step 2: Collect AGENTS.md files
...
```
Glob("~/.codex/projects/*/rules/**/*.md")
```

If any exist, read them. Record same fields plus which project they belong to.

Also check active repos for local `.codex/rules/` directories. To find repo paths, parse the project directory names under `~/.codex/projects/` (they encode the repo path with hyphens). Only check repos that are likely to exist on disk.
...
Content in AGENTS.md that duplicates what rule files already cover. AGENTS.md is always loaded; rule files are also always loaded for the relevant scope. Duplication wastes tokens.
...
|-------|-------|-------|---------------------------|
| Global rules | N | N | N |
| Global AGENTS.md | 1 | N | N |
| Project AGENTS.md files | N | N | N |
| Project rules | N | N | N |
| **Total** | **N** | **N** | **N** |
...
Scanned: {N} global rules, {N} AGENTS.md files, {N} project rule overrides
...
Categories appear in this order: Contradictions, Redundancy, Shadowing, Staleness, AGENTS.md overlap.
