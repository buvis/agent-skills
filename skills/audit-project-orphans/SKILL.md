...
description: >-
  Use when the user wants to find stale or orphaned project configs in ~/.codex/projects/.
...
Check whether the source directory for each project config in `~/.codex/projects/` still exists on disk. Flag orphans and dormant projects.

## Step 1: List project directories

```
Bash: ls ~/.codex/projects/
```
...
If `~/.codex/projects/<dir>/sessions-index.json` exists, read it and extract the `projectPath` field from the first entry. This is the authoritative source path.
...
4. If the candidate does not exist, try common dot-prefix corrections. The encoding turns `/.` into `--` (two consecutive hyphens become `/` + `/` which is wrong - the second segment should be dot-prefixed). Scan the decoded path for segments that look like they should be dot-prefixed: `Codex`, `config`, `local`, `vifm`, `ssh`, `gnupg`, `cache`. For each `--` in the original directory name, try replacing the decoded `//` with `/.` and re-check with `test -d`.
...
Suggested action: Review orphans and remove with:
  rm -rf ~/.codex/projects/<orphan-dir>
```
...
2. Archive orphans to `~/.codex/projects-archive/` (move, not copy)
3. Do nothing
