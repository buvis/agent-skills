...
`run._survey` returns.  The brief is ephemeral, so there is no atlas.json, no
atlas.md, and no staleness flag to assert on.
"""
...
# tree-sitter is an optional dependency. Everything the skill itself owns is
# tested either way; the two cases below assert on what the parser resolves
# (an indented method as kind 'method'), which the regex fallback cannot see.
requires_tree_sitter = pytest.mark.skipif(
    run.try_import_tree_sitter() is None,
    reason="needs the optional tree_sitter_language_pack",
)


# ---------------------------------------------------------------------------
# Pinned contract constants - do NOT alter these strings
# ---------------------------------------------------------------------------

VALID_ERROR_STYLES = {"result", "exceptions", "mixed", "unknown"}
...
def test_survey_writes_nothing_under_home(tmp_path, monkeypatch, capsys):
    """A full `main()` run must leave no store behind anywhere under HOME.

    The map is generated and handed to the session, never stored. An
    implementation that writes an atlas.json, a cache dir, an audit log, or
    even one empty directory fails here.
    """
    repo = tmp_path / "repo"
    repo.mkdir()
    _init_git_repo(repo)
    ...
    assert not list(home.iterdir()), (
        f"survey must not create anything under HOME; found {list(home.iterdir())}"
    )


def test_run_py_executes_standalone(tmp_path):
    """run.py surveys and prints as a plain child process, writing no HOME.

    The rest of the suite imports `run` directly, so it cannot see an
    import-time failure, a missing dependency, or a write the module makes on
    someone else's HOME. This is the only test that runs the real entry point.
    """
    repo = tmp_path / "repo"
    repo.mkdir()
    _init_git_repo(repo)
    ...
    assert "## Where things live" in proc.stdout, \
        f"run.py must print the brief to stdout:\n{proc.stdout}"

    # tree_sitter_language_pack unpacks its compiled grammars into the platform
    # cache dir on first import (~/Library/Caches on macOS, ~/.cache on Linux).
    # That belongs to the dependency; any other file here would be survey's.
    strays = [
        str(p.relative_to(home))
        for p in home.rglob("*")
        if p.is_file() and "tree-sitter-language-pack" not in p.parts
    ]
    assert not strays, f"run.py must not write under HOME; found {strays}"


# ---------------------------------------------------------------------------
# Layers: what is skipped, and what is not
# ---------------------------------------------------------------------------

def test_build_output_is_skipped_but_ordinary_dirs_are_not(tmp_path):
    """Build and dependency output never becomes a layer; anything else does.

    The skip list holds only names that mean build output in every repo, so a
    source directory that happens to be called `tasks` or `logs` keeps its own
    layer. A host-tuned skip list would silently drop it from the brief.

    The directory is deliberately not a git repo: survey reads the tree, never
    the index, so a plain directory must produce a brief just the same.
    """
    repo = tmp_path / "repo"
    repo.mkdir()
    for name in ("services", "tasks", "logs", "node_modules", "dist", "target"):
        (repo / name).mkdir()
        (repo / name / "thing.py").write_text("def thing():\n    pass\n")

    section = _section_body(run._survey(repo), "Where things live")

    for kept in ("services", "tasks", "logs"):
        assert f"**{kept}**" in section, \
            f"{kept!r} is source, not build output, and must be its own layer:\n{section}"
    for skipped in ("node_modules", "dist", "target"):
        assert f"**{skipped}**" not in section, \
            f"{skipped!r} is build output and must never become a layer:\n{section}"
...
def test_error_style_value_is_in_allowed_enum(git_repo):
    ...
    assert match.group(1) in VALID_ERROR_STYLES, \
        f"error_style {match.group(1)!r} must be one of {VALID_ERROR_STYLES}"
...
def test_degraded_not_reported_when_tree_sitter_available(git_repo, monkeypatch):
    """The degraded note must be absent when tree-sitter is importable.

    Fails against an implementation that hardcodes degraded = True. A stand-in
    module drives the branch, so the assertion holds on a host without the
    optional package too: the flag keys on importability, not on parse results.
    """
    monkeypatch.setattr(run, "try_import_tree_sitter", lambda: object())
