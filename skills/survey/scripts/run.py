...
try:
    import tree_sitter_language_pack as _TREE_SITTER
except ImportError:
    # Optional. Absent means symbols come from the regex extractor and the
    # brief says so. The import system caches the result for the process.
    _TREE_SITTER = None


def try_import_tree_sitter() -> ModuleType | None:
    """The tree-sitter module, or None when the optional package is absent."""
    return _TREE_SITTER
...
# Build and dependency output: not source, and a layer made out of one is pure
# noise. Only names that mean the same thing in every repo belong here - a
# skip list tuned to one host silently drops real source layers elsewhere.
# (Dot-dirs like .venv and .git are already skipped separately.)
...
def _scan_layers(repo_path: Path) -> tuple[dict[str, list[Path]], bool]:
    ...
    for d in top_dirs:
        ...
        for dirpath, dirnames, filenames in os.walk(d):
            ...
            # special files included; is_file() filters those out.
            for fn in filenames:
