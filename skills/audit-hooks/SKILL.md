...
- Direct script path: `~/.codex/hooks/foo.sh` - check that path
- Interpreter prefix: `python3 ~/.codex/hooks/foo.py` - check the script path (second token), not the interpreter
...
- For interpreter-prefix commands: use the script name, e.g., `python3 ~/.codex/hooks/foo.py` becomes "foo"
