...
- Files read from other skill dirs:
  ...
  - `~/.agents/skills/run-autopilot/scripts/detect_usage_limit.py` - optional,
...
**Detect** it by scanning the run's output / `-o` file for the same markers `detect_usage_limit.py` uses: `hit your (session|usage|weekly) limit` or `usage limit reached`, and `resets <HH[:MM]> <am|pm> (<tz>)` for the reset time. For a deterministic reset epoch, run `python3 ~/.agents/skills/run-autopilot/scripts/detect_usage_limit.py --log <the -o output file>` — exit 0 prints the reset epoch (it parses both the prose banner and a stream-json `resetsAt`); exit 1 means no live limit.
