...
# - the 2026-07 prose-era reports: exclusion line
...
def discover_repos() -> tuple[list[Path], str]:
    ...
    try:
        ...
        if proc.returncode != 0:
            note = f"gita freeze exited {proc.returncode}; scanning the working directory"
    except (OSError, subprocess.TimeoutExpired) as exc:
        rows, note = [], f"gita unavailable ({exc}); scanning the working directory"
    for row in rows:
        parts = row.split(",")
        if len(parts) >= 3 and parts[2]:
            paths.append(Path(parts[2]))
    # A host's own config directory can itself carry autopilot telemetry, but
