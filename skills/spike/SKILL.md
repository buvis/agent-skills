...
compatibility: "Portable build-first discovery; needs only git and a host that can run an attended build loop."
---
...
- Plugin skills (`autopilot` plugin), named only as the pipeline the graduated PRD
  later travels, never invoked here: `autopilot:plan-tasks`, `autopilot:work`.
  Without the plugin the spike still graduates to a PRD; you implement it by hand.
